...
const extractJson = (filePath) => {
    try {
        ...
        if (obj.result) {
            const match = obj.result.match(/<untrusted-data-[^>]+>\s*([\s\S]*?)\s*<\/untrusted-data-[^>]+>/);
            if (match) {
                return JSON.parse(match[1]);
            }
        }
    } catch (e) {
        console.error('Error on file', filePath, e);
    }
    return null;
};

const basePath = `C:/Users/ASUS/.gemini/antigravity/brain/ce690af0-b223-4010-9f63-e53ea489fb91/.system_generated/steps`;
...
for (const p of paths) {
    ...
    if (fs.existsSync(fullPath)) {
        const parsed = extractJson(fullPath);
...
fs.writeFileSync('d:/Hackathon/LoginPTN/supabase/migrations/004_data_migration.sql', sqlDoc);
